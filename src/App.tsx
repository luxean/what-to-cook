...
import { Archive, ArrowLeft, Check, ChefHat, Cloud, CloudOff, History, Leaf, LoaderCircle, LogOut, MoreHorizontal, Pencil, Plus, RotateCcw, Sparkles, Trash2, X } from 'lucide-react'
...
export function App() {
  ...
  const [vegetarianOnly, setVegetarianOnly] = useState(() => localStorage.getItem('co-navarit-vegetarian-only') === 'true')
  const active = useMemo(() => meals.filter(m => !m.archived), [meals])
  const candidates = useMemo(() => active.filter(meal => !vegetarianOnly || meal.vegetarian), [active, vegetarianOnly])
  ...
  useEffect(() => { localStorage.setItem('co-navarit-vegetarian-only', String(vegetarianOnly)) }, [vegetarianOnly])
  ...
  const reject = () => {
    ...
    setSuggestion(recommend(updated.filter(meal => !meal.archived && (!vegetarianOnly || meal.vegetarian)), nextExcluded))
  }
  const accept = () => {
    if (!suggestion) return
  ...
  const changeDiet = (vegetarian: boolean) => {
    setVegetarianOnly(vegetarian)
    setExcluded([])
    setSuggestion(null)
  }
  const saveMeal = (name: string, cookedToday = false, vegetarian = false) => {
    const clean = name.trim(); if (!clean) return
    if (editing) setMeals(items => items.map(m => m.id === editing.id ? { ...m, name: clean, vegetarian } : m))
    else setMeals(items => {
      const existing = cookedToday ? items.map(meal => ({ ...meal, cookedDates: meal.cookedDates.filter(date => !isToday(Number(date))) })) : items
      return [...existing, { id: crypto.randomUUID(), name: clean, vegetarian, cookedDates: cookedToday ? [String(Date.now())] : [], rejectionDates: [], consecutiveRejections: 0, archived: false, createdAt: String(Date.now()) }]
    })
  ...
  return <div className="app-shell">
    ...
    <main>
      {screen === 'home' && <Home activeCount={active.length} candidateCount={candidates.length} archivedCount={meals.length - active.length} vegetarianOnly={vegetarianOnly} todayMeal={todayMeal} exhausted={exhausted} suggestion={suggestion} onDietChange={changeDiet} onSuggest={() => suggest()} onAccept={accept} onReject={reject} onUndoToday={undoToday} onResetSuggestions={resetSuggestions} onMeals={() => setScreen('meals')} onArchive={() => setScreen('archive')} />}
    ...
    {(showAdd || editing) && <MealDialog initial={editing?.name} initialVegetarian={editing?.vegetarian} onSave={saveMeal} onClose={() => { setShowAdd(false); setEditing(null) }} />}
    {toast && <div className="toast"><Check size={18}/>{toast}</div>}
  </div>
}

function Home({ activeCount, candidateCount, archivedCount, vegetarianOnly, todayMeal, exhausted, suggestion, onDietChange, onSuggest, onAccept, onReject, onUndoToday, onResetSuggestions, onMeals, onArchive }: { activeCount: number; candidateCount: number; archivedCount: number; vegetarianOnly: boolean; todayMeal: Meal | null; exhausted: boolean; suggestion: ScoredMeal | null; onDietChange: (vegetarian: boolean) => void; onSuggest: () => void; onAccept: () => void; onReject: () => void; onUndoToday: () => void; onResetSuggestions: () => void; onMeals: () => void; onArchive: () => void }) {
  return <section className="home">
    <div className="diet-filter" aria-label="Filter jedál"><button className={!vegetarianOnly ? 'selected' : ''} onClick={() => onDietChange(false)}>Všetky</button><button className={vegetarianOnly ? 'selected' : ''} onClick={() => onDietChange(true)}><Leaf/> Vegetariánske</button></div>
    {todayMeal ? <div className="suggestion-wrap chosen-wrap">
      ...
      {activeCount > 0 && !candidateCount && <p className="empty-note">Nemáš uložené žiadne vegetariánske jedlá.</p>}
...
function MealList({ meals, cloudEnabled, syncing, onLogout, onAdd, onEdit, onDelete, onArchive, onArchiveScreen }: { meals: Meal[]; cloudEnabled: boolean; syncing: boolean; onLogout: () => void; onAdd: () => void; onEdit: (m: Meal) => void; onDelete: (id: string) => void; onArchive: (id: string) => void; onArchiveScreen: () => void }) {
  return <section><div className="section-head"><p>Jedlá, z ktorých vyberáme</p><button className="primary compact" onClick={onAdd}><Plus/> Pridať</button></div>
    <div className="meal-list">{meals.map(meal => <article className="meal-row" key={meal.id}><div className="meal-avatar">{meal.name[0]?.toUpperCase()}</div><div className="meal-info"><strong>{meal.name}{meal.vegetarian && <Leaf className="veg-leaf"/>}</strong><small>{meal.cookedDates.length ? `Uvarené ${meal.cookedDates.length}×` : 'Ešte neuvarené'}</small></div><button className="tiny-btn" onClick={() => onEdit(meal)} aria-label="Upraviť"><Pencil/></button><button className="tiny-btn" onClick={() => onArchive(meal.id)} aria-label="Archivovať"><Archive/></button><button className="tiny-btn danger" onClick={() => confirm(`Naozaj vymazať „${meal.name}“?`) && onDelete(meal.id)} aria-label="Vymazať"><Trash2/></button></article>)}</div>
...
function ArchiveList({ meals, onRestore, onDelete }: { meals: Meal[]; onRestore: (id: string) => void; onDelete: (id: string) => void }) {
  return <section><p className="page-copy">Jedlá, na ktoré teraz nemáš chuť. Kedykoľvek ich môžeš vrátiť.</p><div className="meal-list">{meals.map(meal => <article className="meal-row" key={meal.id}><div className="meal-avatar muted">{meal.name[0]}</div><div className="meal-info"><strong>{meal.name}{meal.vegetarian && <Leaf className="veg-leaf"/>}</strong><small>{meal.consecutiveRejections ? `${meal.consecutiveRejections}× odmietnuté` : 'Archivované'}</small></div><button className="tiny-btn restore" onClick={() => onRestore(meal.id)} aria-label="Obnoviť"><RotateCcw/></button><button className="tiny-btn danger" onClick={() => confirm(`Naozaj vymazať „${meal.name}“?`) && onDelete(meal.id)} aria-label="Vymazať"><Trash2/></button></article>)}</div>{!meals.length && <Empty text="Archív je prázdny." />}</section>
}
